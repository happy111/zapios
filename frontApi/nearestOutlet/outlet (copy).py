...
class RestaurantMapView(LoggingMixin, APIView):
	...
	def merge_all_nearest_restaurents(self, restaurant, kilometers):
		try:
			...
			restaurant_data['area'] = restaurant.location
			restaurant_data['city'] = restaurant.city
			...
			restaurant_data['latitude'] = restaurant.latitude
			restaurant_data['country'] = CountryMaster.objects.filter(id=restaurant.country_id)[0].country
			...
			restaurant_data['time_range'] = restaurant.time_range
			now = datetime.now()
			today = now.strftime('%A')
			tdata = OutletTiming.objects.filter(Company_id=self.Company,day=today)
			if tdata.count() > 0:
				restaurant_data["outlet_time"] = []
				for index in tdata:
					di = {}
					di['open_time'] = index.opening_time
					di['close_time'] = index.closing_time
		...
		if self.customer_nearest_restaurent == []:
			...
			for restaurant in self.Restaurants:
				...
				if unloaded_mile == None:
					...
					print("pppppppppppppppppppppp",chk_out)
