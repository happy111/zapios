...
class RetrieveLocality(APIView):

	"""
	Locality POST API

		Authentication Required		: No
		Service Usage & Description	: This Api is used for retrieval of Locality data.
		...
		Response: {
			...
			"success"  : True, 
			"message"  : "locality retrieval api worked well!!",
			"data"     : final_result
		}
	...
	def post(self, request, format=None):
		try:
			...
			if record.count() == 0:
				...
				q_dict["area"]          = record[0].area				
				q_dict['city'] = []
				dict_city = {}
				dict_city['value'] = record[0].city_id
				dict_city['label'] = CityMaster.objects.filter(id=record[0].city_id)[0].city
				q_dict['city'].append(dict_city)
			...
			return Response({
						"success": True, 
						"message": gettext_lazy("Area retrieval api worked well!!"),
						"data": final_result,
						})
