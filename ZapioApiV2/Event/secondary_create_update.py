...
from Event.models import PrimaryEventType,SecondaryEventType

class EventSerializer(serializers.ModelSerializer):
	class Meta:
		model = SecondaryEventType
		fields = '__all__'

class SecondaryEventCreationUpdation(APIView):

	"""
	Secondary Event Creation & Updation POST API
		...
		Data Post: {
			...
			"event_type"		       : "1",
			"secondary_event"          : "Sale"
		}
		Response: {

			"success": True, 
			"message": "Secondary Event type creation/updation api worked well!!",
			"data": final_result
		}
	...
	def post(self, request, format=None):
		try:
			...
			data['company'] = cid
			err_message = {}
			err_message["secondary_event"] = only_required(data["secondary_event"],"Secondary Event Type")
			...
			if "id" in data:
				unique_check = SecondaryEventType.objects.filter(~Q(id=data["id"]),\
								Q(event_type=data["event_type"]),\
								Q(company_id=cid),\
								Q(secondary_event=data['secondary_event']))
			else:
				unique_check = SecondaryEventType.objects.filter(Q(event_type=data["event_type"]),\
									Q(company_id=cid),Q(secondary_event=data['secondary_event']))
			...
			if "id" in data:
				event_record = SecondaryEventType.objects.filter(id=data['id'])
