...
class ActionCity(APIView):

	"""
	City Action POST API
		...
		Response: {
			...
			"success": True, 
			"message": "City is deactivated now!!",
			"data": final_result
		}
	...
	def post(self, request, format=None):
		try:
			...
			if record.count() != 0:
				...
				if data["active_status"] == "true":
					info_msg = "City is activated successfully!!"
				else:
					info_msg = "City is deactivated successfully!!"
				serializer = CitySerializer(record[0],data=data,partial=True)
				...
				return Response(
					{
						"success": False,
						"message": "City id is not valid to update!!"
					}
					)
			final_result = []
			...
			print("City action Api Stucked into exception!!")
			print(e)
