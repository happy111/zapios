	...
	def post(self, request, format=None):
		try:
			...
			if record.count() != 0:
				...
				if data["active_status"] == "true":
					info_msg = "Payment is activated successfully!!"
				else:
					info_msg = "Payment is deactivated successfully!!"
				serializer = PaymentSerializer(record[0],data=data,partial=True)
				...
				return Response(
					{
						"success": False,
						"message": "Payment id is not valid to update!!"
					}
					)
			final_result = []
