		...
		try:
			...
			if query.count()==0:
				...
				for index in query:
					...
					q_dict["order_source"] = []
					ut = index.id
					source_data = OrderSource.objects.filter(Q(payment_method__contains=[ut]),
						Q(company_id=cid))
					if source_data.count() > 0:
						for i in source_data:
							dic = {}
							source_id = OrderSource.objects.filter(source_name=i)[0].id
							dic['id'] = source_id
							dic['order_source'] = OrderSource.objects.filter(source_name=i)[0].source_name
