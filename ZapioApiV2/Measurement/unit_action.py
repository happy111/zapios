...
class ActionUnit(APIView):

	"""
	Unit Action POST API
		...
		Response: {
			...
			"success": True, 
			"message": "Unit is deactivated now!!",
			"data": final_result
		}
	...
	def post(self, request, format=None):
		try:
			...
			if record.count() != 0:
				...
				if data["active_status"] == "true":
					info_msg = "Unit is activated successfully!!"
				else:
					info_msg = "Unit is deactivated successfully!!"
				serializer = UnitSerializer(record[0],data=data,partial=True)
				...
				return Response(
					{
						"success": False,
						"message": "Unit id is not valid to update!!"
					}
					)
			final_result = []
			...
			print("Unit action Api Stucked into exception!!")
			print(e)
