	...
	def get(self, request,format=None):
		try:
			...
			if report_type == 'Consoledate':
				...
				ws.write(0, 0, 'Start Date',xlwt.easyxf('align:horiz centre; pattern: pattern solid, fore_color gray25;  font: color black,bold on,height 200'))
				ws.write(0, 1, s_date)

				ws.write(0, 2, 'End Date',xlwt.easyxf('align:horiz centre; pattern: pattern solid, fore_color gray25;  font: color black,bold on,height 200'))
				ws.write(0, 3, e_date)
					...
					for i in staff_data:
						...
						for index in query:
							...
							if index.time_out != None:
								...
								p_list['clock_out'] = out_time.strftime("%d/%b/%y %I:%M %p")
							else:
								p_list['clock_out'] = '' 
							if index.time_in != None:
								in_time = index.time_in+timedelta(hours=5,minutes=30)
								p_list['clock_in'] = in_time.strftime("%d/%b/%y %I:%M %p")
							else:
								p_list['clock_in'] = ''
							if index.status == 'present':
								p_list['status'] = 'Present'
			...
			if report_type == 'Individual':
				...
				for index in query:
					...
					row = [
						staff_id,
						str(staff_data[0].manager_name) + str(staff_data[0].last_name),
						staff_data[0].user_type.user_type,
						outlet_name,
						p_list['clock_in'],
						p_list['clock_out'],
						p_list['status'],
