...
from Configuration.models import DeliverySetting,PaymentDetails,ColorSetting,AnalyticsSetting
...
class DeliveryEdit(APIView):
	...
	def post(self, request, format=None):
		try:
			...
			# 	record = DeliverySetting.objects.filter(Q(id=data['delivery_id']),Q(company=Company_id))
			# 	if record.count() == 0:
			# 		return Response(
			# 			{
			# 				"status": False,
