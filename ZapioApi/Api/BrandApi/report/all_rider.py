    ...
    def get(self, request, format=None):
            ...
            if report_type == 'consoledate':
                ...
                for obj in ord_data:
                    ...
                    if obj['compensation'] != None and obj['distance'] != None:
                        delivery_cost = float(obj['distance']) * float(obj['compensation'])
                    else:
                        delivery_cost = 0
                    row = [
                        order_date,
            ...
            if report_type == 'rider':
                rider = request.GET.get('staff')
                data = Order.objects.filter(Q(order_time__lte=end_date),Q(order_time__gte=start_date)
                        ,Q(Company_id=cid),Q(is_rider_assign=1),Q(delivery_boy_id=rider))
                response = HttpResponse(content_type='application/ms-excel')
                response['Content-Disposition'] = 'attachment; filename=rider_reports.xls'
                wb = xlwt.Workbook(encoding='utf-8')
                ws = wb.add_sheet("rider_reports")
            ...
            if report_type == 'outlet':
                ...
                for k in outletss:
                    data = Order.objects.filter(Q(order_time__lte=end_date),Q(order_time__gte=start_date)
                            ,Q(Company_id=cid),Q(is_rider_assign=1),outlet=k)
                    if data.count() > 0:
                        for i in data:
                            p_list = {}
                            p_list['order_time'] = i.order_time
